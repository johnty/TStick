...
void Wifimanager_init(bool WiFiSerialDebug) {
  ...
  if (SPIFFS.begin()) {
    ...
    ESP_LOGD("wifi_man", "mounted FS...");
    
    if (SPIFFS.exists("/config.json")) {
      //file exists, reading and loading
      ESP_LOGD("wifi_man", "reading config file...");
      File configFile = SPIFFS.open("/config.json", "r");
      if (configFile) {
        ESP_LOGD("wifi_man", "opened config file");
        ...
        if (json.success()) {
          ...
          ESP_LOGD("wifi_man", "failed to load JSON config...");
        }
      }
    }
  } else {
    ESP_LOGD("wifi_man", "failed to mount FS");
